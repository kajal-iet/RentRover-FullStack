...
const Products = mongoose.model('Products', { pname: String,pdesc:String ,price:String,category:String,pimage:String,pcity:String,addedBy: mongoose.Schema.Types.ObjectId,});
const Users = mongoose.model('Users', { email: String,password:String ,mobile:String,username:String,cartItems:[{type:mongoose.Schema.Types.ObjectId,ref:"Products"}]});
...
app.post('/addproduct',upload.single('pimage'),(req,res)=>{
    ...
    const product = new Products({ pname: pname,pdesc:pdesc,price:price,category:category,pimage:pimage,pcity:pcity,addedBy:addedBy });
    product.save()
    .then(()=>{
...
//   console.log(productId)
  ...
  app.post('/remove-from-cart', async (req, res) => {
    ...
    if (!userId || !productIdToRemove) {
        return res.status(400).send({ message: 'Bad request. Missing user ID or product ID.' });
    }
